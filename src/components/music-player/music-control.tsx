...
const Progress = ({ value }: { value: number }) => {
  return (
    <div className="w-full h-[4px] bg-black relative">
      <div
        className="bg-gray-300 w-0 h-full absolute left-0 top-0 transition-[width] "
        style={{ width: `${value}%` }}
      ></div>
    </div>
  )
        ...
        <div className="w-full h-full lg:w-[920px] lg:px-6 px-2 mx-auto flex items-center bg-white">
          ...
          <div className="ml-4 h-full flex-1 flex flex-col justify-between">
            <div className="flex items-center justify-between">
              <span
                className="max-w-[158px] sm:max-w-[68%] overflow-ellipsis overflow-hidden whitespace-nowrap sm:text-base text-sm font-bold"
                title={audioState.name}
              >
                {audioState.name || '请选择歌曲'}
              </span>
              <span className="ml-4 text-xs">
                {timeFormatter(Math.floor(audioState.currentTime))} / {timeFormatter(Math.floor(audioState.duration))}
              </span>
            </div>
